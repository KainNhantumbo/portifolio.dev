...
function FluentIconButton({
  ...
  return (
    <motion.button
      ...
      className={cn(
        `group/icon-button hover:bg-[var(--icon-button-color)]/10 active:bg-[var(--icon-button-color)]/20 relative inline-flex size-10 shrink-0 cursor-pointer rounded-full text-[var(--icon-button-color)]`,
        sizes[size],
        className
      )}
      ...
      {...props}>
      <motion.div
        className='stroke-muted-foreground absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 group-hover/icon-button:stroke-[var(--icon-button-color)]'
        aria-hidden='true'>
      ...
      <AnimatePresence mode='wait'>
        {active && (
          <motion.div
            className='absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 fill-[var(--icon-button-color)] text-[var(--icon-button-color)]'
            aria-hidden='true'
...
export { FluentIconButton, sizes, type IconButtonProps };
